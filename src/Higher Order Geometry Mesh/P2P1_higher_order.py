...
geometry_tag = "higher_order_geom"
...
for refinement_level in range(35):
    ...
    side_length = r * (np.sin(np.pi * 2 / (3 + refinement_level))) / np.sin((np.pi - (np.pi * 2) / (3 + refinement_level)) / (2)) # length of the side of the square
    ...
    if mesh_comm.rank == model_rank:    
        ...
        num_points_on_cylinder = 4 + refinement_level # Example: 4 is the minumum nuber of points 
        for curve_tag in obstacle:
            gmsh.model.mesh.setTransfiniteCurve(curve_tag, num_points_on_cylinder)
        ...
        gmsh.model.mesh.field.setNumber(threshold_field, "LcMin", side_length) # change to side_length. res_min is the min mesh size at the distMin
        ...
        gmsh.model.mesh.field.setNumber(threshold_field, "DistMax", 2 * H) # changed from 2 * H to H. above this element will have size 0.25 * H
        max_field = gmsh.model.mesh.field.add("Max") 
        gmsh.model.mesh.field.setNumbers(max_field, "FieldsList", [threshold_field]) 
        gmsh.model.mesh.field.setAsBackgroundMesh(max_field) # sets the thereshhold field for the whole domain
